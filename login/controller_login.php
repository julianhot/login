<?php 
	...
	// verifica si la variable al registrarse está definida
	...
	if (isset($_POST['registrarse'])) {
		...
		// si el id del objeto retornado no es null, 
		//quiere decir que encontro un registro en la base
		if ($usuario->getId()!=NULL) {
			$_SESSION['usuario']=$usuario; 
			// si el usuario se encuentra, crea la sesión de usuario
			header('Location: cuenta.php'); 
			// envia a la página que simula la cuenta
		}else{
	...
	}elseif(isset($_POST['salir'])){ // para que salga del home
		header('Location: index.php');
		unset($_SESSION['usuario']); // destruye la sesión actual uwu
	}
?>